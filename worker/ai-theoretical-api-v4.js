 ...
 * MODIFICATO v4.0: aggiunto campo track obbligatorio
 */
async function handleSubmit(request, env, corsHeaders) {
  const formData = await request.formData();
  
  // Validazione campi obbligatori - AGGIUNTO track
  ...
  // Costruisci oggetto submission - AGGIUNTO track
  const submission = {
    id,
    paperToken,
    confirmToken,
  ...
  // Invia email di conferma - AGGIUNTO track
...
/**
 * Genera pagina HTML di conferma - AGGIUNTO track
 */
...
/**
 * Invia email di conferma via Resend - AGGIUNTO track
 */
