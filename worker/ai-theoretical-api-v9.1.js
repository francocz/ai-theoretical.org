 ...
 * 
 * Changes in v9.1:
 *   - Added GET /author-access/:token - HTML page for managing papers
 ...
 *   GET  /author-access/:token          - Pagina gestione paper autore (pubblico, token required)
 * 
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // GET /author-access/:token - Pagina gestione paper (pubblico)
      const authorAccessMatch = path.match(/^\/author-access\/([a-f0-9-]+)$/);
...
async function sendAuthorAccessEmail(env, to, papers, accessToken) {
  const accessUrl = `https://ai-theoretical.org/author-access/${accessToken}`;
...
async function handleAuthorAccessPage(token, env) {
  ...
  if (!tokenData) {
    return new Response(renderAuthorAccessPage(null, 'expired'), {
      status: 400,
  ...
  return new Response(renderAuthorAccessPage(data, 'valid'), {
    status: 200,
    headers: { 'Content-Type': 'text/html; charset=utf-8' }
  });
}
 ...
 */
function renderAuthorAccessPage(data, status) {
  if (status === 'expired') {
    return `<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script>
    ...
    async function confirmWithdraw() {
      ...
      status.textContent = 'Withdraw functionality coming soon. Please contact support for now.';
    ...
    async function confirmNewVersion() {
      ...
      // TODO: Implement actual new version API call
      // For now, show placeholder message
      status.className = 'status-message error';
      status.textContent = 'New version submission coming soon. Please contact support for now.';
